...
for hd in blockdevices["blockdevices"]:
    for part in hd["children"]:
        ...
        print('<action name="Execute"><execute>x-terminal-emulator -e "sudo mkdir -p /media/'+part["uuid"]+' &amp;&amp; sudo mount -o umask=000 '+part["name"]+' /media/' + part["uuid"] + ' ||  pcmanfm /media/'+''+'"</execute></action>')
        print ('</item>')
print ('</openbox_pipe_menu>')
